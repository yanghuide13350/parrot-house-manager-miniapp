...
Page({
  ...
  async onLoad(options: any) {
    ...
    try {
      const plan = (await repository.feedingPlans()).find((item: any) => item.id === options.id)
      if (!plan) { wx.showToast({ title: '方案不存在或已删除', icon: 'none' }); return }
      if (plan.isEnabled) { wx.showToast({ title: '请先关闭方案后再编辑', icon: 'none' }); setTimeout(() => backOrSwitchTab('/pages/feeding-plans/feeding-plans'), 400); return }
      this.setData({ plan, isEdit: true, form: Object.keys(emptyForm()).reduce((result: any, key) => ({ ...result, [key]: String(plan[key] ?? '') }), {}) })
    } catch (error: any) { wx.showToast({ title: error.message || '加载方案失败', icon: 'none' }) }
  },
  ...
  chooseFeedingType(event: any) { this.setData({ 'form.feedingType': event.currentTarget.dataset.type }) },
  goStep(event: any) { const step = Number(event.currentTarget.dataset.step); if (step >= 1 && step <= 3) this.setData({ step }) },
  previousStep() { if (this.data.step > 1) this.setData({ step: this.data.step - 1 }) },
  nextStep() {
    const form: any = this.data.form
    if (this.data.step === 1 && (!form.name || !form.species || !form.stage)) { wx.showToast({ title: '请填写方案名称、品种和阶段', icon: 'none' }); return }
    if (this.data.step < 3) this.setData({ step: this.data.step + 1 })
    else this.save()
  },
  async save() {
    const form: any = this.data.form
    if (!form.name || !form.species || !form.stage || this.data.submitting) return
    const input: any = { ...form }
    numberKeys.forEach(key => input[key] = Number(input[key] || 0))
    this.setData({ submitting: true })
    try {
      if (this.data.isEdit) await repository.updateFeedingPlan(this.data.plan, input)
      else await repository.createFeedingPlan(input)
      wx.showToast({ title: '方案已保存', icon: 'success' })
      setTimeout(() => backOrSwitchTab('/pages/feeding-plans/feeding-plans'), 500)
    } catch (error: any) { wx.showToast({ title: error.message || '保存失败', icon: 'none' }) } finally { this.setData({ submitting: false }) }
  },
  remove() {
    if (!this.data.plan || this.data.submitting) return
    if (this.data.plan.isEnabled) { wx.showToast({ title: '请先关闭方案后再删除', icon: 'none' }); return }
    wx.showModal({ title: '删除喂养方案', content: '删除后无法恢复，已关联的鸟将自动解除该方案。确认删除？', success: async result => {
      if (!result.confirm) return
      this.setData({ submitting: true })
      try { await repository.deleteFeedingPlan(this.data.plan); wx.showToast({ title: '已删除', icon: 'success' }); setTimeout(() => backOrSwitchTab('/pages/feeding-plans/feeding-plans'), 500) }
