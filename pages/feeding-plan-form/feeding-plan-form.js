...
Page({
    ...
    input(event) { const key = event.currentTarget.dataset.key; let value = event.detail.value; if (ageDayKeys.has(key) && value !== '')
        value = String(Math.min(30, Math.max(0, Number(value) || 0))); this.setData({ [`form.${key}`]: value }); if (key === 'species')
