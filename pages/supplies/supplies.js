...
Page({
    data: { activeTab: 'SUPPLY', supplyMonths: [], medicineMonths: [], currentMonths: [], currentLabel: '耗材', currentIcon: 'package', showForm: false, submitting: false, todayDate: today(), form: emptyForm() },
    onShow() { this.reload(); },
    async reload() {
        try {
            const records = await repository_1.repository.supplies();
    ...
    async save() {
        ...
        try {
            await repository_1.repository.createSupply({ ...form, category: this.data.activeTab });
            await this.reload();
            this.setData({ showForm: false });
