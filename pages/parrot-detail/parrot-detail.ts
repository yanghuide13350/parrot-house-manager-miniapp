...
Page({
  data: { parrot: null as any, genderLabel: '', media: [] as any[], clutches: [] as any[], visibleClutches: [] as any[], clutchTotal: 0, showAllClutches: false, activeIndex: 0, showPairing: false, showSale: false, showProgress: false, showFeedingPlan: false, showFeedingPicker: false, feedingPlans: [] as any[], feedingPlan: null as any, eligible: [] as any[], selectedMate: '', selectedFeedingPlan: '', eggCount: '3', buyer: '', price: '', contact: '', shareToken: '', shareUrl: '', shareExpiryLabel: '', submitting: false, detailTopStyle: '', detailActionsStyle: '' },
  ...
  onShow() { this.syncTopBar(); this.restoreShareState(); if (this.id) { this.refresh(); this.loadFeedingPlans() } },
  syncTopBar() {
  ...
  async loadFeedingPlans() { try { const feedingPlans = await repository.feedingPlans(), current = store.getParrot(this.id), currentAge = current ? ageDays(current.birthDate) : -1, feedingPlan = feedingPlans.find(item => item.id === current?.feedingPlanId && item.isEnabled) || feedingPlans.find(item => item.isEnabled && item.species === current?.breed && currentAge >= planStart(item) && currentAge <= planEnd(item)) || null; this.setData({ feedingPlans, feedingPlan }) } catch (error: any) { wx.showToast({ title: error.message || '喂养方案加载失败', icon: 'none' }) } },
  openFeedingPlan() { if (this.data.feedingPlan) this.setData({ showFeedingPlan: true }); else this.openFeedingPicker() },
  closeFeedingPlan() { this.setData({ showFeedingPlan: false }) },
  openFeedingPicker() { this.setData({ showFeedingPlan: false, showFeedingPicker: true, selectedFeedingPlan: this.data.parrot.feedingPlanId || this.data.feedingPlan?.id || '' }) },
  closeFeedingPicker() { if (!this.data.submitting) this.setData({ showFeedingPicker: false }) },
  chooseFeedingPlan(event: any) { this.setData({ selectedFeedingPlan: event.currentTarget.dataset.id }) },
  async confirmFeedingPlan() { if (!this.data.selectedFeedingPlan || this.data.submitting) return; this.setData({ submitting: true }); try { await store.setFeedingPlan(this.id, this.data.selectedFeedingPlan); const feedingPlan = this.data.feedingPlans.find((item: any) => item.id === this.data.selectedFeedingPlan) || null; this.setData({ showFeedingPicker: false, feedingPlan }); wx.showToast({ title: '已选择喂养方案', icon: 'success' }) } catch (error: any) { wx.showToast({ title: error.message || '设置失败', icon: 'none' }) } finally { this.setData({ submitting: false }) } },
