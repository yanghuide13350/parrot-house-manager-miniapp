...
Page({
  ...
  onLoad(options: any) {
    ...
    else if (!configured) this.setData({ configured: false, title: '主账号未配置', detail: '复制下面的 OpenID，填入 Worker 的 OWNER_OPENID 后重新部署。', eyebrow: 'OWNER SETUP', openId, cta: '复制 OpenID', helper: '配置完成后重新进入即可。' })
    else if (accessStatus === 'pending') this.setData({ accessStatus, openId, requestNote, title: '申请审核中', detail: '管理员正在审核你的访问申请，审核通过后重新进入即可。', eyebrow: 'REQUEST PENDING', cta: '查看申请状态', helper: requestNote ? `申请备注：${requestNote}` : '你也可以把 OpenID 发给管理员加快处理。' })
    else if (accessStatus === 'rejected') this.setData({ accessStatus, openId, requestNote, reviewNote, title: '申请未通过', detail: '你可以补充备注后重新提交申请，管理员会再次收到待审批记录。', eyebrow: 'REQUEST REJECTED', cta: '重新提交申请', helper: reviewNote ? `拒绝原因：${reviewNote}` : '可补充用途说明后重新提交。' })
    else this.setData({ accessStatus, openId, requestNote, title: '申请访问权限', detail: '当前微信账号未获得授权，可直接提交申请给管理员审批。', eyebrow: 'ACCESS REQUIRED', cta: '申请访问权限', helper: '也可以先复制 OpenID 发给管理员。' })
  },
  inputNote(event: any) { this.setData({ note: event.detail.value }) },
  ...
  async submitRequest() {
    ...
    if (this.data.submitting || this.data.accessStatus === 'pending') return
    ...
    try {
      await repository.requestAccess(note)
