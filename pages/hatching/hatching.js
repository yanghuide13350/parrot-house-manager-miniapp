...
Page({
    ...
    parseOffspringGroups() {
        ...
        const groups = lines.map(line => { const match = line.match(/^(.+?)\s*[：:\s]\s*(\d+)\s*(只)?$/); if (!match)
            throw new Error('请按“品种 数量”逐行填写'); return { species: match[1].trim(), count: Number(match[2]) }; });
        const total = groups.reduce((sum, item) => sum + item.count, 0);
        if (total > this.data.updateValue)
            throw new Error('品种数量不能超过出壳数量');
        return groups;
    },
    ...
    complete(event) { if (this.data.submitting)
        return; const id = event.currentTarget.dataset.id; wx.showModal({ title: '确认完成', content: '完成后将保留父母鸟的配对关系，可继续下一轮孵化。', success: async (result) => { if (!result.confirm || this.data.submitting)
    ...
    viewParrot(event) { const id = String(event.currentTarget.dataset.id || ''); if (!id || !store_1.store.getParrot(id))
