import { GenderCode, ParrotStatusCode, PLACEHOLDER_IMAGE } from '../../utils/types'
...
Page({
  ...
  complete(event: any) { if (this.data.submitting) return; const id = event.currentTarget.dataset.id; wx.showModal({ title: '确认完成', content: '完成后将保留父母鸟的配对关系，可继续下一轮孵化。', success: async (result: any) => { if (!result.confirm || this.data.submitting) return; this.setData({ submitting: true }); try { await store.completeHatching(id); wx.showToast({ title: '孵化任务已完成', icon: 'success' }) } catch (error: any) { wx.showToast({ title: error.message || '操作失败', icon: 'none' }) } finally { this.setData({ submitting: false }) } } }) },
