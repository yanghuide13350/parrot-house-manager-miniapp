...
Page({
  data: { records: [] as any[], search: '', showAdd: false, showUpdate: false, updateRecord: null as any, updateValue: 0, submitting: false, newRecord: { species: '', maleId: '', femaleId: '', startDate: todayDate(), eggs: '3' }, breeders: [] as any[], maleBreeders: [] as any[], femaleBreeders: [] as any[] },
  ...
  remove(event: any) { if (this.data.submitting) return; const id = event.currentTarget.dataset.id; wx.showModal({ title: '确认删除', content: '记录会从列表隐藏并保留审计。', success: async (result: any) => { if (!result.confirm || this.data.submitting) return; this.setData({ submitting: true }); try { await store.deleteHatching(id); wx.showToast({ title: '记录已移除', icon: 'none' }) } catch (error: any) { wx.showToast({ title: error.message || '删除失败', icon: 'none' }) } finally { this.setData({ submitting: false }) } } }) },
  noop() {},
