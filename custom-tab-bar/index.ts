...
Component({
  ...
  detached() { if (this.tabTapTimer) clearTimeout(this.tabTapTimer) },
  ...
  methods: {
    ...
    switchTab(event: any) {
      if (this.tabTapTimer) return
      this.tabTapTimer = setTimeout(() => { this.tabTapTimer = null }, 500)
      const path = String(event.currentTarget.dataset.path || '')
      const pages = getCurrentPages(); const current = pages[pages.length - 1] as any
      if (current?.route === path) {
        if (typeof current.refreshFromTab === 'function') current.refreshFromTab()
        return
      }
      wx.switchTab({ url: `/${path}` })
    }
  },
  tabTapTimer: null as any
})
