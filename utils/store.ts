...
class Store {
  ...
  async updateHatching(id: string, updates: Partial<HatchingRecordDoc>) { const record = this.hatchingRecords.find(item => item.id === id); if (!record) throw new Error('找不到孵化记录'); if (updates.hatched == null) throw new Error('出壳数量无效'); await repository.updateHatching(record, updates.hatched, updates.offspringGroups || []); await this.hydrate(true) }
  ...
  async createFromClutch(id: string, chicks: any[]) { const record = this.hatchingRecords.find(item => item.id === id); if (!record) throw new Error('找不到孵化记录'); const result = await repository.createFromClutch(record, chicks); await this.hydrate(true); return result }
